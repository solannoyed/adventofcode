...
export default function (input: string) {
	...
	while (queue.length > 0) {
		const current = queue.shift()!; // make sure the BFS!
		if (coolest[current.y][current.x][current.direction] <= current.heat) continue;
		coolest[current.y][current.x][current.direction] = current.heat;
...
type Crucible = {
	...
	// travelled: number; // blocks travelled in the same direction
