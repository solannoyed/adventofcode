...
var getAnswer = function (filename) {
	...
	let round = 0;
	while (true) {
		// implement a check for if it has spread out enough. or we could have a boolean for if anything ended up moving
		...
		for (const elf of elves) {
			...
			for (let index = 0; index < DIRECTIONS.length; index++) {
				// N/S/E/W
				let blocked = false;
				for (const direction of DIRECTIONS[(round + index) % 4]) {
					// eg NW/N/NE
					if (locations.has(`${elf.x + direction.x},${elf.y + direction.y}`)) {
						blocked = true;
						break;
