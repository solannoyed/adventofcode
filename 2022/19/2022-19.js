...
var getAnswer = function(filename) {
	...
	let blueprints = data.trimEnd().split('\n').map((line) => {
...
var getMaxGeodeCountGreedy = function(bp) {
	...
	for (let minute = 0; minute < 24; minute ++) {
		...
		if (
			...
			bp.geode_robots ++;
			building = true;
			console.log(`Spend ${bp.geode_robot_ore} ore and ${bp.geode_robot_obsidian} obsidian to build a geode-collecting robot.`);
		}
		// ---- OBSIDIAN ROBOT
		if (
			...
			if (
				(bp.geode_robot_obsidian - bp.obsidian) * bp.ore_robots > (bp.geode_robot_ore + bp.obsidian_robot_ore - bp.ore) * bp.obsidian_robots
			) {
				bp.ore -= bp.obsidian_robot_ore;
				bp.clay -= bp.obsidian_robot_clay;
				bp.obsidian_robots ++;
				building = true;
				console.log(`Spend ${bp.obsidian_robot_ore} ore and ${bp.obsidian_robot_clay} clay to build an obsidian-collecting robot.`);
			}
		}
		// ---- CLAY ROBOT
		if (
			...
			if (
				(bp.obsidian_robot_clay - bp.clay) * bp.ore_robots > (bp.obsidian_robot_ore + bp.clay_robot_ore - bp.ore) * bp.clay_robots
			) {
...
var getMaxGeodeCountDFS = function(blueprint, minutes = 24) {
	...
	while (stack.length > 0) {
		...
		if (maxBp && bp.minute >= 20) {//} && bp % 4 === 0) { // if these checks are 'expensive' we might only want to do it every few minutes
			// console.log('test2');
			...
			if (bp.geode + (bp.geode_robots * (minutes - bp.minute)) + MAX_GEODES[minutes - 0 - bp.minute] <= maxBp.geode) continue;
		}
		...
		if (bp.obsidian_robots <= bp.geode_robot_obsidian && (bp.waiting === WAITING.NONE || bp.waiting === WAITING.OBSIDIAN)) {
		...
		if (
			...
			(bp.clay_robots > 0 && (bp.ore - bp.ore_robots < bp.obsidian_robot_ore || bp.clay - bp.clay_robots < bp.obsidian_robot_clay)) || // if we have clay production but not enough ore or clay for obsidian robot
			(bp.obsidian_robots > 0 && (bp.ore - bp.ore_robots < bp.geode_robot_ore || bp.obsidian - bp.obsidian_robots < bp.geode_robot_obsidian))// if we have obsidian production but not enough ore or clay for geode robot
		) {
			bp.order += '- ';
			stack.push(bp);
		}
		...
		if ( // we have enough to build whatever we want
